...
def is_between_100_and_999(user_input_number):
    ...
    result =  user_input_number_int < 1000 and user_input_number_int >= 100
...
def is_duplicated_number(three_digit):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    three_digit_int = int(three_digit)
    first_num = three_digit_int // 100
    three_digit_int = three_digit_int % 100
    second_num = three_digit_int // 10
    third_num = three_digit_int % 10
    result = (not first_num is second_num and not second_num is third_num and not first_num is third_num)
    # ==================================
    return result
...
def get_strikes_or_ball(user_input_number, random_number):
    ...
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    result = [0, 0]
    for user_index, user_val in enumerate(user_input_number):
        for ran_index, ran_val in enumerate(random_number):
            if (user_val == ran_val):
                if (user_index == ran_index):
                    result[0] += 1
                else:
                    result[1] += 1
